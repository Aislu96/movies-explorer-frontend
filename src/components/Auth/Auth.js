...
function Auth({title, name, button, authText, authTextLink, handleSubmit, children, to, classButton, isValid, errorMessage}) {
    return (
        <div className="auth">
            <Link to="/" className="auth__logo-link"><img src={logo} className="auth__logo" alt="Логотип проекта Movies"/></Link>
            ...
            <p className="auth__text">{authText}<Link to={to} className="auth__text-link">{authTextLink}</Link></p>
        </div>
    );
}
